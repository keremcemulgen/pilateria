...
setTimeout(()=>{ try {
  ...
  w.eval(`
    ...
    __mk('z','member','u9','',['u9'],'${P3}',['completed','completed','completed','completed','completed','completed','completed','completed']); // P3 8/8 -> COK ESKI Bitti, listede DEGIL
  `);
  w.renderDashboard();
  ...
  t('cok eski (3 ay) bitmis uye listede DEGIL (ZUHAL)', !/ZUHAL/.test(lh));
  ...
  t('📦 yalniz sarkanlarda (3 adet: gpm, gbitpm, u8)', lh.split('📦').length - 1 === 3, lh.split('📦').length - 1);
  const satirlar = lh.split('class="row between"').length - 1;
  t('7 satir (gpm,gbitpm,gcm + u1,u2,u7,u8)', satirlar === 7, satirlar);
