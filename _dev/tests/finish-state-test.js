...
setTimeout(()=>{ try {
  ...
  const ay=w.eval('currentMonth()'); // 2026-08-02: tarih-dayaniklilik
  // Kerem ekrani: 8 kota, 8 ders girilmis — 0 yapildi, 8 planli (paket 17.07-15.08 devam ediyor)
  w.eval(`
    const M=id=>({id,name:id.toUpperCase(),joinDate:'2026-01-01',archived:false,totalPrice:4500,packages:[],monthly:{'${ay}':{enrolled:true}}});
    state.members=[M('c1'),M('c2'),M('c3'),M('c4')];
    state.groups=[{id:'G',name:'G',size:4,memberIds:['c1','c2','c3','c4'],monthlyMembers:{'${ay}':['c1','c2','c3','c4']},
      packages:[{month:'${ay}',startDate:'${ay}-17',sessions:8,status:'active'}],defaultTime:'19:00'}];
    state.lessons=Array.from({length:8},(_,i)=>({id:'L'+i,date:'${ay}-'+String(17+i).padStart(2,'0'),time:'19:00',
      status:'planned',instructorId:'h',groupId:'G',memberIds:['c1','c2','c3','c4'],packageMonth:'${ay}',
  ...
  d.getElementById('group-month') && (d.getElementById('group-month').value=ay);
  w.renderGroups();
  ...
  w.eval(`state.lessons.push({id:'BI',date:'${ay}-20',time:'10:00',status:'planned',instructorId:'h',groupId:'',memberIds:['c1'],packageMonth:'${ay}',packageOwnerType:'member',packageOwnerId:'c1',size:1});`);
