...
setTimeout(()=>{ try {
  ...
  const ay=w.eval('currentMonth()'); // 2026-08-02: tarih-dayaniklilik (calisma ayina gore)
  w.eval(`
    ...
    state.members.push({id:'a2',name:'A (2. Paket)',secondOfMember:'a',joinDate:'2026-01-01',archived:false,totalPrice:4500,packages:[],monthly:{'${ay}':{enrolled:true}}});
    state.groups=[]; state.lessons=[]; state.payments=[];
    document.getElementById('dash-month') && (document.getElementById('dash-month').value='${ay}');
  `);
  ...
  w.eval("state.members.find(x=>x.id==='a').monthly['"+ay+"']={enrolled:false};");
  w.eval("try{renderDashboard()}catch(e){}");
